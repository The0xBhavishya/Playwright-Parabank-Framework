...
> 10 |     expect(response.status()).toBe(200);
     |                               ^ Error: expect(received).toBe(expected) // Object.is equality
  11 |     expect(body).toContain(`<customerId>${customerId}</customerId>`);
  12 |     
  13 |     expect(body.includes("<type>CHECKING</type>") ||body.includes("<type>SAVINGS</type>")).toBeTruthy();
  14 |     const balance =body.match(/<balance>(.*?)<\/balance>/)?.[1];
  15 |     expect(isNaN(Number(balance))).toBeFalsy();
  16 | 
  17 | });
```