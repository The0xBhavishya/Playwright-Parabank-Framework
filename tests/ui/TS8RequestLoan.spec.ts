...
import RequestLoan from "../../pages/requestLoan.page";

test("TS15 Request Loan", async ({ page }) => {
    const login = new Login(page);
    const requestLoan = new RequestLoan(page);
    await login.navigate();
    await login.loginUser();
    await requestLoan.requestLoan("1000", "100");
    await requestLoan.verifyLoanRequestProcessed();
    const loanAccountNo = await requestLoan.getLoanAccountNumber();
    console.log("Generated Loan Account Number :", loanAccountNo);
    await requestLoan.screenshot();
    await login.logout();
});
...
// test('S8 Before After Balance Comparison', async({page,request}) =>{
